...
export function BudgetView({ rerender }) {
  ...
  const remaining = budgets.__remaining__ || 0;                          // money you allow yourself outside the category budgets
  const allocated = items.reduce((s, i) => s + (i.budget || 0), 0);
  const total = allocated + remaining;
  const spent = total ? Object.values(cur).reduce((s, v) => s + v, 0) : 0;   // ALL spend (incl. Other) counts against the total
  ...
  const overall = total ? el('section', { class: 'panel overall' },
    ...
    el('div', { class: 'muted alloc' }, `${money0(allocated)} in category budgets + `, remainingEditor(), ` remaining = ${money0(total)}`),
    ...
    : el('div', { class: 'empty' }, 'No budgets yet. Tap + budget on a category, then set a remaining amount: ', remainingEditor());
  function remainingEditor() {
    if (store.state.editing === '__remaining__') {
      const input = el('input', { type: 'number', min: 0, step: 10, value: remaining || '', placeholder: '$', style: { width: '90px', padding: '2px 6px' },
        onkeydown: e => { if (e.key === 'Enter') save('__remaining__', +input.value); if (e.key === 'Escape') { store.state.editing = null; rerender(); } } });
      setTimeout(() => input.focus(), 0);
      return el('span', { class: 'row', style: { display: 'inline-flex', gap: '4px' } }, input, el('button', { class: 'primary', style: { padding: '2px 8px' }, onclick: () => save('__remaining__', +input.value) }, 'Save'));
    }
    return el('button', { class: 'budget-btn', onclick: () => { store.state.editing = '__remaining__'; rerender(); } }, remaining ? `${money0(remaining)} ✎` : '+ remaining');
  }
  return el('div', {},
