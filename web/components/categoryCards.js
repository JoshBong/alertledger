...
function BudgetBar({ spent, budget, elapsed }) {
  ...
  const status = over ? `over by ${money0(spent - budget)}` : left === 0 ? 'spent exactly' : `${money0(left)} left`;
  const paceTxt = pace != null && !over ? ` · pace ${money0(pace)}` : '';
  return el('div', { class: 'budget' },
    ...
    el('div', { class: 'sub' }, el('span', {}, `${money0(spent)} of ${money0(budget)}`), el('span', { class: over ? 'up' : ahead ? 'warn' : '' }, status + paceTxt)));
}
