...
export function Donut({ items, total, title, titleClass = '', subtitle, onSelect, selected = null }) {
  ...
  if (items.length === 1) {                                                  // a 360° arc is degenerate in SVG: draw a ring
    const it = items[0];
    const ring = svg('circle', { cx: C, cy: C, r: (R + r) / 2, fill: 'none', stroke: it.color, 'stroke-width': R - r, class: selected && it.key !== selected ? 'dim' : '', onclick: () => onSelect?.(it.key) });
  ...
  for (const it of items.length === 1 ? [] : items) {
    ...
    const path = svg('path', { d, fill: it.color, class: selected ? (it.key === selected ? 'on' : 'dim') : '', onclick: () => onSelect?.(it.key) });
