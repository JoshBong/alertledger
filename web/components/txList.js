...
// TxList({ rows, colorFor(cat), limit, drag }) — rows newest first; grouped by day.
// `drag` (from lib/recat.js) makes each row draggable into another category.
export function TxList({ rows, colorFor, limit = 400, drag }) {
  ...
  for (const t of rows.slice(0, limit)) {
    ...
    const cat = t.ignore ? 'ignored' : t.type === 'transfer' ? 'income' : t.amount > 0 && t.type === 'zelle' ? t.cat + ' · paid back' : t.cat;
    const row = el('div', { class: 'tx' + (drag ? ' grab' : '') + (t.pinned ? ' pinned' : '') },
